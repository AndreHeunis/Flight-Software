...
#include "includes.h"
#include "bsp_rtc.h"
#include "bsp_uart.h" // included for printing out in unix epoch time conversion function
#include <time.h>
...
void BSP_RTC_Init ()
{
	...
	uint32_t ticksPerMs = CMU_ClockFreqGet(cmuClock_RTC) / 1000; // number of ticks for increment interval
	RTC_CompareSet(0, ticksPerMs);
